...
The CMS_get0_type() function returns a pointer to the content type of a CMS_ContentInfo structure as
an ASN1_OBJECT pointer.  This is typically set when the CMS_ContentInfo structure is created, or when a signed or encrypted artifact is decoded.
...
content type.  It differs from CMS_get0_type in that it points to the actual
eContentType element contained in the ASN1 structure that was constructed or received.
It should return a single encoded OID.

CMS_get0_content() returns a pointer to the B<ASN1_OCTET_STRING> pointer
containing the embedded content itself.
It's use is undefined on outgoing objects created with B<CMS_PARTIAL>
until L<CMS_final> is called.
