...
ssl.h includes a macro OPENSSL_VERSION_AT_LEAST(maj,min,fix), which can be used in #if statements in order to determine which version of the library is
being used. This can be used to either enable optional features at compile time,
or work around issues with a previous version.
